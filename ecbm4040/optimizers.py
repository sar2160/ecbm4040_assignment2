...
class RMSpropOptim(Optimizer):
    ...
    def step(self, model, learning_rate):
        ...
        for k in grads:
            ...
            delta_param = - (learning_rate / np.sqrt(cache[k]+eps)) * grads[k]
            params[k] += delta_param
...
class AdamOptim(Optimizer):
    ...
    def step(self, model, learning_rate):
        ...
        for k in grads:
            ...
            v_tilde = velocitys[k] / (1 - np.power(beta2, t))
            m_tilde = momentums[k] / (1 - np.power(beta1, t))
            delta_param = - (learning_rate * m_tilde / (np.sqrt(v_tilde)+ eps))
            params[k] += delta_param
